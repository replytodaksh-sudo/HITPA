...
const DocumentUpload: React.FC<DocumentUploadProps> = ({
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (response?.statusCode === 0 && response?.payload?.length > 0) {
                ...
                console.log('Uploaded Doc ID:', documentsCodes);
    ...
    const handleOpenPreview = async (doc: DocumentData) => {
        ...
        setPreviewFile(`${apiUrls.viewDocument}${doc.fileLocation}&documentId=${doc.documentID}`);
        setPreviewOpen(true);
    };
...
// Converted from Angular CentralRegionalDocumentsComponent
