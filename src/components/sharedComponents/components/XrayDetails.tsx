...
//     setFormState(newFormState);
//     const fieldNames = getFieldNames(sectionKey);
//     console.log("1234567890", fieldNames[fieldKey])
...
interface SectionConfig {
  value: string;
  label: string;
  visitDoneField: keyof HospitalFormState;
  reportsVerifiedField: keyof HospitalFormState;
  findingField: keyof HospitalFormState;
}
...
const SECTIONS: SectionConfig[] = [
  {
    value: 'sonography',
    label: 'Sonography',
    visitDoneField: 'sonographyVisitDone',
    reportsVerifiedField: 'sonographyReportsVerified',
    findingField: 'sonographyFinding',
  },
  {
    value: 'xray',
    label: 'X-Ray',
    visitDoneField: 'xrayVisitDone',
    reportsVerifiedField: 'xrayReportsVerified',
    findingField: 'xrayFinding',
  },
  {
    value: 'mri',
    label: 'MRI',
    visitDoneField: 'mriVisitDone',
    reportsVerifiedField: 'mriReportsVerified',
    findingField: 'mriFinding',
  },
  {
    value: 'ctscan',
    label: 'CT Scan',
    visitDoneField: 'ctScanVisitDone',
    reportsVerifiedField: 'ctScanReportsVerified',
    findingField: 'ctScanFinding',
  },
  {
    value: 'bloodbank',
    label: 'Blood Bank Visit',
    visitDoneField: 'bloodBankVisitDone',
    reportsVerifiedField: 'bloodBankReportsVerified',
    findingField: 'bloodBankFinding',
  },
  {
    value: 'physiotherapy',
    label: 'Physiotherapy Centres',
    visitDoneField: 'therapyVisitDone',
    reportsVerifiedField: 'therapyReportsVerified',
    findingField: 'therapyFinding',
  },
];
...
const XrayDetails: React.FC<XrayDetailsProps> = ({
  ...
  // Local state for checkbox visibility
  ...
  // Initialize visibility based on formState
  useEffect(() => {
    const newVisibility: Record<string, boolean> = {};
    
    SECTIONS.forEach(section => {
      // Show section if visitDone field has a value (true or false, not null)
      newVisibility[section.value] = formState[section.visitDoneField] !== null;
    });
  ...
  const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setVisibility({ ...visibility, [value]: checked });
  };

  // Handle field update
  const handleFieldUpdate = (
    field: keyof HospitalFormState,
    value: any
  ) => {
  ...
  const renderSection = (section: SectionConfig) => {
    if (!visibility[section.value]) return null;

    const visitDone = formState[section.visitDoneField];
    const reportsVerified = formState[section.reportsVerifiedField];
    const finding = formState[section.findingField];

    return (
      <Box key={section.value} sx={{ mb: 4 }}>
        <Typography variant="h6" sx={{ mb: 2, fontWeight: 'bold' }}>
          {section.label}
        </Typography>
        ...
        <Grid container spacing={3} sx={{ mb: 2 }}>
          ...
          <Grid size={{ xs: 12, md: 3 }}>
            <RadioGroup
              row
              value={visitDone === null ? '' : String(visitDone)}
              onChange={(e) =>
                handleFieldUpdate(section.visitDoneField, e.target.value === 'true')
              }
            >
              <FormControlLabel
                value="true"
          ...
          <Grid size={{ xs: 12, md: 3 }}>
            <RadioGroup
              row
              value={reportsVerified === null ? '' : String(reportsVerified)}
              onChange={(e) =>
                handleFieldUpdate(section.reportsVerifiedField, e.target.value === 'true')
              }
            >
              <FormControlLabel
                value="true"
        ...
        <Grid container spacing={3}>
          ...
          <Grid size={{ xs: 12, md: 9 }}>
            <TextField
              ...
              rows={4}
              value={finding || ''}
              onChange={(e) => handleFieldUpdate(section.findingField, e.target.value)}
              disabled={!isFormEditable}
            />
          </Grid>
        </Grid>
            ...
            {SECTIONS.map((section) => (
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    value={section.value}
                    checked={visibility[section.value] || false}
                    onChange={handleCheckboxChange}
                    disabled={!isFormEditable}
                  />
                }
