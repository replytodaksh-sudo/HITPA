...
//   // Populate data on mount and when previousData changes
//   useEffect(() => {
//     if (previousData) {
//       populateData();
//     }
...
//     // Set chemist data array
//     const data = previousData.reCaseUpdateChemistDetailsDTO || [];
//     setChemistData(data);
//     caseUpdateService.setCaseUpdateVal('reCaseUpdateChemistDetailsDTO', data);

//     // Set chemist exists
//     if (previousData.chemistExist === true) {
//       setIsChemistExists('1');
//       caseUpdateService.setCaseUpdateVal('chemistExist', true);
//     } else if (previousData.chemistExist === false) {
//       setIsChemistExists('0');
//       caseUpdateService.setCaseUpdateVal('chemistExist', false);
//       setChemistExistFinding(previousData.chemistExistFinding || '');
//       caseUpdateService.setCaseUpdateVal('chemistExistFinding', previousData.chemistExistFinding);
//     }
//   };
...
//   // Handle chemist exists change - matches Angular's chemistExists(event)
//   const handleChemistExistsChange = (value: string) => {
//     setIsChemistExists(value);
...
//     } else if (value === '0') {
//       caseUpdateService.setCaseUpdateVal('chemistExist', false);
//       caseUpdateService.setCaseUpdateVal('chemistExistFinding', chemistExistFinding);
//     }
//   };

//   // Handle chemist exist finding change - matches Angular's changechemistExistFinding(e)
//   const handleChemistExistFindingChange = (value: string) => {
//     setChemistExistFinding(value);
//     caseUpdateService.setCaseUpdateVal('chemistExistFinding', value);
//   };
...
//   // Add chemist - matches Angular's addChemistDetails(id:string)
//   const handleAddChemist = () => {
//     const newChemist: ChemistDetails = {
//       chemistName,
//       chemistAddress,
...
//     // Add to array and update service
//     const updatedChemistData = [...chemistData, newChemist];
//     setChemistData(updatedChemistData);
//     caseUpdateService.setCaseUpdateVal('reCaseUpdateChemistDetailsDTO', updatedChemistData);
...
//   // Matches Angular's changeVal(value:string)
//   const changeVal = (value: boolean | undefined): string => {
//     if (value === true) return 'Yes';
//     if (value === false) return 'No';
//     return '';
...
//                       value={chemistCity}
//                       onChange={(e) => setChemistCity(e.target.value)}
//                       disabled={!chemistState} // Disable if no state selected
//                     >
//                       <MenuItem value="">Select</MenuItem>
//                       {cities.map((city) => (
//                         <MenuItem key={city.cityCode} value={city.cityCode}>
...
const ChemistDetailsComponent: React.FC<ChemistDetailsComponentProps> = ({
  ...
  // Handle chemist exists change
  const handleChemistExistsChange = (value: string) => {
    if (value === '1') {
      updateFormState('chemistExist', true);
    } else if (value === '0') {
      updateFormState('chemistExist', false);
    }
  };

  // Handle chemist exist finding change
  const handleChemistExistFindingChange = (value: string) => {
    updateFormState('chemistExistFinding', value);
  };
  ...
  const handleAddChemist = () => {
    ...
    const updatedChemistData = [...(formState.reCaseUpdateChemistDetailsDTO || []), newChemist];
    updateFormState('reCaseUpdateChemistDetailsDTO', updatedChemistData);
  ...
  // Get chemist exists value for radio
  const getChemistExistsValue = () => {
    if (formState.chemistExist === true) return '1';
    if (formState.chemistExist === false) return '0';
    return '';
  };

  const chemistData = formState.reCaseUpdateChemistDetailsDTO || [];
  ...
  return (
    <Box sx={{ mt: 3 }}>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          ...
          <Grid size={{ xs: 12, md: 9 }}>
            <TextField
              ...
              multiline
              rows={7}
              value={formState.chemistExistFinding || ''}
              onChange={(e) => handleChemistExistFindingChange(e.target.value)}
              disabled={!buttonEnable}
            />
          </Grid>
      ...
      {/* Chemist Table */}
      {formState.chemistExist === true && chemistData.length > 0 && (
        <TableContainer component={Paper} sx={{ mb: 3, maxHeight: 500, overflow: 'auto' }}>
          <Table stickyHeader size="small">
            <TableHead>
              <TableRow sx={{ backgroundColor: '#6F62C2' }}>
