...
const AgencyInvestigationCompletedCashless: React.FC = () => {
  ...
  const getReAssignClaims = async (role?: string) => {
    try {
      setLoading(true);
      let response: any = await claimsService.getAgencyCompletedCases();
  ...
  const getCasesByTatRange = async (selectedTatRange: string) => {
    ...
    try {
      setLoading(true);
      let response: any = await claimsService.getAgencyCompletedCasesByTat(selectedTatRange);
  ...
  const handleViewClick = (investigationID: string, claim: ReassignClaim) => {
    navigate(
      `/admin/agency-case-not-investigated/${investigationID}${investigationID}?redirectTo=${location.pathname}&claimsType=cashless&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}`,
      {
        state: {
...
export default AgencyInvestigationCompletedCashless;