...
const FieldReworkCasesCashless: React.FC = () => {
  ...
  const getReworkClaims = async (role?: string) => {
    try {
      ...
      if (currentRole === 'Regional Manager') {
        response = await claimsService.getReworkClaimsReg();
      } else if (currentRole === 'Agency Spoc') {
        response = await claimsService.getReworkClaims();
      }
  ...
  const getCasesByTatRange = async (selectedTatRange: string) => {
    ...
    try {
      ...
      if (currentRole === 'Regional Manager') {
        response = await claimsService.getReworkClaimsRegByTat(selectedTatRange);
      } else if (currentRole === 'Agency Spoc') {
        response = await claimsService.getReworkClaimsByTat(selectedTatRange);
      }
...
export default FieldReworkCasesCashless;