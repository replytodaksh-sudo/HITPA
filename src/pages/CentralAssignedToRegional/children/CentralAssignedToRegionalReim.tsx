...
const CentralAssignedToRegionalReim: React.FC = () => {
  ...
  const fetchClaims = async () => {
    setLoading(true);
    try {
      const response = await claimsService.fetchAllCentralAssignedToRegionalReims();
  ...
  const handleViewClick = (investigationID: string, claim: Claim) => {
    navigate(`/admin/central-assign-regional/${investigationID}?redirectTo=${location.pathname}&claimsType=reim&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}`);
  };
...
export default CentralAssignedToRegionalReim;