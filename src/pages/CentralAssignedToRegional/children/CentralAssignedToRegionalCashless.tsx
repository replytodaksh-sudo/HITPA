...
const CentralAssignedToRegionalCashless: React.FC = () => {
  ...
  const fetchClaims = async () => {
    setLoading(true);
    try {
      const response = await claimsService.fetchAllCentralAssignedToRegional();
  ...
  const handleViewClick = (investigationID: string, claim: Claim) => {
    navigate(`/admin/central-assign-regional/${investigationID}?redirectTo=${location.pathname}&claimsType=cashless&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}`);
  };
...
export default CentralAssignedToRegionalCashless;