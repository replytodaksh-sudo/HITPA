...
const RegCentralQueryCashless: React.FC = () => {
    ...
    const fetchClaims = async () => {
        setLoading(true);
        try {
            const response = await claimsService.fetchCentralQueryCashless();
    ...
    const getCasesByTatRange = async (tatValue: string) => {
        ...
        setLoading(true);
        try {
            const response: any = await claimsService.fetchCentralQueryCashlessByTat(tatValue);
            if (response.statusCode === 0) {
                setClaims(response.payload);
                setFilteredClaims(response.payload);
            }
    ...
    const handleViewClick = (investigationID: string, claim: CashlessClaim) => {
        navigate(
            `/admin/central-query-form/${investigationID}?claimsType=cashless&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}`
        );
    };
...
export default RegCentralQueryCashless;