...
const QCPenCashless: React.FC = () => {
    ...
    const fetchClaims = async () => {
        setLoading(true);
        try {
            const response = await claimsService.fetchPendingFromCentralCashless();
    ...
    const getCasesByTatRange = async (tatValue: string) => {
        ...
        setLoading(true);
        try {
            const response: any = await claimsService.fetchPendingFromCentralCashlessByTat(tatValue);
            if (response.statusCode === 0) {
                setClaims(response.payload);
                setFilteredClaims(response.payload);
            }
    ...
    const handleViewClick = (investigationID: string, claim: CashlessClaim) => {
        navigate(
            `/admin/qc-pending-central-form/${investigationID}?claimsType=cashless&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}&claimSubmitted=true`
        );
    };
