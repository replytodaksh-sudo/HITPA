...
const AgencyCaseNotInvestigatedCashless: React.FC = () => {
  ...
  const getReAssignClaims = async (role?: string) => {
    try {
      setLoading(true);
      let response: any = await claimsService.getAgencyNotCompletedCases();
  ...
  const getCasesByTatRange = async (selectedTatRange: string) => {
    ...
    try {
      setLoading(true);
      let response: any = await claimsService.getAgencyNotCompletedCasesByTat(selectedTatRange);
...
export default AgencyCaseNotInvestigatedCashless;