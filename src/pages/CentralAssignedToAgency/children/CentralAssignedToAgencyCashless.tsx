...
const CentralAssignedToAgencyCashless: React.FC = () => {
  ...
  const fetchClaims = async () => {
    setLoading(true);
    try {
      const response = await claimsService.fetchAllCentralAssignedToAgency();
  ...
  const handleViewClick = (investigationID: string, claim: Claim) => {
    navigate(`/admin/central-assign-agency/${investigationID}?redirectTo=${location.pathname}&claimsType=cashless&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}`);
  };
...
export default CentralAssignedToAgencyCashless;