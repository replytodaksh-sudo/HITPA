...
const CentralAssignedToAgencyReim: React.FC = () => {
  ...
  const fetchClaims = async () => {
    setLoading(true);
    try {
      const response = await claimsService.fetchAllCentralAssignedToAgencyReims();
  ...
  const handleViewClick = (investigationID: string, claim: Claim) => {
    navigate(`/admin/central-assign-agency/${investigationID}?redirectTo=${location.pathname}&claimsType=reim&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}`);
  };
...
export default CentralAssignedToAgencyReim;