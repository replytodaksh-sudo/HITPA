...
const CentralCaseNotInvestigatedCashless: React.FC = () => {
  ...
  const fetchClaims = async () => {
    setLoading(true);
    try {
      const response = await claimsService.fetchAllCentralCaseNotInvestigated();
  ...
  const handleViewClick = (investigationID: string, claim: Claim) => {
    navigate(`/admin/regional-case-not-inv-form/${investigationID}?redirectTo=${location.pathname}&claimsType=cashless&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}`)
  };
...
export default CentralCaseNotInvestigatedCashless;