...
const RegQcPendingReim: React.FC = () => {
    ...
    const fetchClaims = async () => {
        setLoading(true);
        try {
            const response = await claimsService.fetchQCPendingClaimsReim();
    ...
    const getCasesByTatRange = async (tatValue: string) => {
        ...
        setLoading(true);
        try {
            const response: any = await claimsService.fetchQCPendingClaimsReimByTat(tatValue);
            if (response.statusCode === 0) {
                setClaims(response.payload);
                setFilteredClaims(response.payload);
            }
...
export default RegQcPendingReim;