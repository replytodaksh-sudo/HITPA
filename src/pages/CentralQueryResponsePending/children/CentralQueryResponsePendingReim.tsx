...
const CentralQueryResponsePendingReim: React.FC = () => {
    ...
    const getReworkClaims = async (role?: string) => {
        try {
            setLoading(true);
            let response: any = await claimsService.fetchAllCentralQueryResponsePendingReim();
    ...
    const getCasesByTatRange = async (selectedTatRange: string) => {
        ...
        try {
            setLoading(true);
            let response: any = await claimsService.fetchAllCentralQueryResponsePendingReimByTat(selectedTatRange);
    ...
    const handleViewClick = (investigationID: string, claim: ReimClaim) => {
        navigate(
            `/admin/central-qc-form/${investigationID}?redirectTo=${location.pathname}&claimsType=reim&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}&acceptAssignId=${claim.acceptAssignId}`,
            {
                state: {
                    claimsType: 'reim',
...
export default CentralQueryResponsePendingReim;