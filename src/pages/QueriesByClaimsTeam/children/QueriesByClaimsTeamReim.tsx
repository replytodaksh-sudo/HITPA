...
const QueriesByClaimsTeamReim: React.FC = () => {
    ...
    const getReworkClaims = async (role?: string) => {
        try {
            setLoading(true);
            let response: any = await claimsService.fetchAllQueriesByClaimsTeamReim();
    ...
    const getCasesByTatRange = async (selectedTatRange: string) => {
        ...
        try {
            setLoading(true);
            let response: any = await claimsService.fetchAllQueriesByClaimsTeamReimByTat(selectedTatRange);
    ...
    const handleViewClick = (investigationID: string, claim: ReimClaim) => {
        navigate(
            `/admin/central-assign-agency/${investigationID}?redirectTo=${location.pathname}&claimsType=reim&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}&acceptAssignId=${claim.acceptAssignId}`,
            {
                state: {
                    claimsType: 'reim',
...
export default QueriesByClaimsTeamReim;