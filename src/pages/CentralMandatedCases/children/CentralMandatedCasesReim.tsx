...
const CentralMandatedCasesReim: React.FC = () => {
  ...
  const loadClaims = async () => {
          ...
          try {
              let response = await claimsService.fetchCentralMandatedCasesReim();
  ...
  const handleViewClick = (investigationID: string, claim: ReimClaim) => {
    navigate(
      `/admin/fresh-central-mandate/${investigationID}?claimsType=reim&claimNo=${claim.tpaClaimNo}&sbigclaimno=${claim.sbigClaimNo}&acceptAssignId=${claim.acceptAssignId}`
    );
  };
...
export default CentralMandatedCasesReim;