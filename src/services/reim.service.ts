...
// File: src/services/reimService.ts
import { apiService } from './api.service';
...
export const ReimService = {
  /**
   * Get tab details for reimbursement claim
   * @param invId - Investigation ID (will be cleaned)
   * @param callType - Call type (tab name or section)
   * @returns Promise with tab details
   */
  getTabDetails: async (
    invId: string,
    callType: string
  ): Promise<ReimResponse> => {
    try {
      // Clean investigation ID (remove suffix after dash)
      const cleanInvId = invId.split('-')[0];
      
      const response = await apiService.get(
        `${apiUrls.getTabDetails}${cleanInvId}&callType=${callType}`
      );
      return response;
    } catch (error) {
      console.error('Error in getTabDetails:', error);
      throw error;
    }
  },
  ...
  caseUpdatePreviousDataReim: async (
    ...
    try {
      ...
      const response = await apiService.get(
        `${apiUrls.caseUpdatePreviousDataReim}${cleanInvId}`
      );
      return response;
    } catch (error) {
